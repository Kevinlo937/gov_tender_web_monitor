...
Fixed title extraction to correctly handle JavaScript-rendered titles.
"""
...
class PCCWebScraper:
    ...
    def _parse_tender_row(self, row):
        ...
        try:
            ...
            # Extract tender ID (first line)
            tender_id = tender_cell.contents[0].strip() if tender_cell.contents else ""
            
            # FIXED: Extract title from JavaScript
            # Look for the script tag that contains the title
            title = "Unknown Title"
            script_tags = tender_cell.select('script')
            for script in script_tags:
                script_text = script.string
                if script_text and 'pageCode2Img' in script_text:
                    # Extract the title from the JavaScript function call
                    title_match = re.search(r'pageCode2Img\("([^"]+)"\)', script_text)
                    if title_match:
                        title = title_match.group(1)
                        break
            
            # If no script tag with title is found, try to find it in any script content
            if title == "Unknown Title":
                html_str = str(tender_cell)
                title_match = re.search(r'pageCode2Img\("([^"]+)"\)', html_str)
                if title_match:
                    title = title_match.group(1)
